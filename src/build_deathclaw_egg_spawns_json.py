#!/usr/bin/env python3
r"""
build_deathclaw_egg_spawns_json.py — generate the spawn JSON for the Deathclaw Egg
Spawn Locations page on buffsnbrew.com.

Covers both Deathclaw Egg (ALCH 00046939, raw consumable) and Cracked Deathclaw Egg
(MISC 0014F6AC, junk item).  Sources include deathclaw nest containers, direct world
spawns, collectrons, and vendor pools.

Modelled on build_nuka_cola_spawns_json.py.  Reads committed game-file exports
(LVLI_Entries / LVLI_Refs / ALCH / MISC) and resolves every world placement via
...
Usage:
    python src/build_deathclaw_egg_spawns_json.py            # live channel
    python src/build_deathclaw_egg_spawns_json.py --pts       # PTS channel
"""
...
from farming_spawns_config import DEATHCLAW_EGG, ALL_REGIONS
...
GEO_CACHE = os.environ.get("FARMING_GEO_CACHE_DEATHCLAW",
                           os.path.join(REPO, "data", "farming_spawns", "geo_cache_deathclaw_egg.json"))
SOURCE_TAG = "Game-file exports (LVLI/ALCH/MISC) + Mappalachia Position (cached for CI)"
SQL_CHUNK = 900
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Build Deathclaw Egg spawn locations JSON")
    ...
    if db_ok:
        ...
        print("[deathclaw-egg-spawns] Mappalachia DB found — resolving placements and refreshing geo cache.")
    elif cache:
        print(f"[deathclaw-egg-spawns] No DB — rebuilding from committed geo cache ({len(cache)} placements).")
    else:
        print("[deathclaw-egg-spawns] No Mappalachia DB and no geo cache — cannot build. "
              "Run once locally with MAPPALACHIA_DB set to seed "
              "data/farming_spawns/geo_cache_deathclaw_egg.json.")
        return

    generated = datetime.date.today().isoformat()
    build(DEATHCLAW_EGG, tbls, geo, cur, cache, db_ok, generated, dist_dir)

    if db_ok:
        save_cache(cache)
        print(f"[deathclaw-egg-spawns] geo cache saved ({len(cache)} placements) for DB-free CI rebuilds.")
    if con:
        con.close()
