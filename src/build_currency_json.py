...
====================
STATUS: SCAFFOLDED — gap filler for the Currency category.

NOTES FOR NEXT AI
─────────────────
1. This script should generate dist/currency.json from xEdit TSV exports.
...
4. EXPECTED OUTPUT:
   dist/currency.json — consumed by the JS module df-bnb-currency.js on the website

5. WORKFLOW:
   There is a matching GitHub Actions workflow:
   .github/workflows/build-currency.yml
...
Usage:
  ...
  python build_currency_json.py --tsv-root tsv --outdir dist
"""
...
def main():
    print(f"[build_currency_json.py] Gap filler — real build logic TBD")

    # Ensure output directory exists
    os.makedirs("dist", exist_ok=True)

    # Write empty/placeholder JSON
    out_path = os.path.join("dist", "currency.json")
    with open(out_path, "w") as f:
        json.dump({"_status": "scaffolded", "_category": "Currency", "items": []}, f, indent=2)

    print(f"[build_currency_json.py] Wrote {out_path}")
