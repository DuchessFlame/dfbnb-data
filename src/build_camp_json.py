...
build_camp_json.py
====================
STATUS: SCAFFOLDED — gap filler for the CAMP category.

NOTES FOR NEXT AI
─────────────────
1. This script should generate dist/camp.json from xEdit TSV exports.
...
4. EXPECTED OUTPUT:
   dist/camp.json — consumed by the JS module df-bnb-camp.js on the website

5. WORKFLOW:
   There is a matching GitHub Actions workflow:
   .github/workflows/build-camp.yml
   that calls this script on push to tsv/ or src/ paths.

6. The master workflow (dfbnb-patch-build.yml) also calls this script.

Usage:
  python build_camp_json.py
  python build_camp_json.py --tsv-root tsv --outdir dist
"""
...
def main():
    print(f"[build_camp_json.py] Gap filler — real build logic TBD")

    # Ensure output directory exists
    os.makedirs("dist", exist_ok=True)

    # Write empty/placeholder JSON
    out_path = os.path.join("dist", "camp.json")
    with open(out_path, "w") as f:
        json.dump({"_status": "scaffolded", "_category": "CAMP", "items": []}, f, indent=2)

    print(f"[build_camp_json.py] Wrote {out_path}")
