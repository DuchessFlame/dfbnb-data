...
====================
STATUS: SCAFFOLDED — gap filler for the Perk Cards category.

NOTES FOR NEXT AI
─────────────────
1. This script should generate dist/perk_cards.json from xEdit TSV exports.
...
4. EXPECTED OUTPUT:
   dist/perk_cards.json — consumed by the JS module df-bnb-perk_cards.js on the website

5. WORKFLOW:
   There is a matching GitHub Actions workflow:
   .github/workflows/build-perk_cards.yml
   that calls this script on push to tsv/ or src/ paths.

6. The master workflow (dfbnb-patch-build.yml) also calls this script.

Usage:
  python build_perk_cards_json.py
  python build_perk_cards_json.py --tsv-root tsv --outdir dist
"""
...
def main():
    print(f"[build_perk_cards_json.py] Gap filler — real build logic TBD")

    # Ensure output directory exists
    os.makedirs("dist", exist_ok=True)

    # Write empty/placeholder JSON
    out_path = os.path.join("dist", "perk_cards.json")
    with open(out_path, "w") as f:
        json.dump({"_status": "scaffolded", "_category": "Perk Cards", "items": []}, f, indent=2)

    print(f"[build_perk_cards_json.py] Wrote {out_path}")

    # Write empty patchlog feed
    write_empty_patchlog_feed("dist", "patchlog_latest_bnb_perk_cards.json", current_count=0)
