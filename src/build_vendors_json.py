#!/usr/bin/env python3
"""
build_vendors_json.py
====================
STATUS: SCAFFOLDED — gap filler for the Vendors category.

NOTES FOR NEXT AI
─────────────────
1. This script should generate dist/vendors.json from xEdit TSV exports.
   NOTE: build_minerva.py covers Minerva vendor specifically. Coordinate
...
4. EXPECTED OUTPUT:
   dist/vendors.json — consumed by the JS module df-bnb-vendors.js on the website

5. WORKFLOW:
   There is a matching GitHub Actions workflow:
   .github/workflows/build-vendors.yml
   that calls this script on push to tsv/ or src/ paths.

6. The master workflow (dfbnb-patch-build.yml) also calls this script.

Usage:
  python build_vendors_json.py
  python build_vendors_json.py --tsv-root tsv --outdir dist
"""
...
def main():
    ...
    out_path = os.path.join("dist", "vendors.json")
