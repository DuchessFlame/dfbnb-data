...
SET_META = {
    "pint-sized-slasher-masks": {
        ...
        # DATED FILENAME on purpose: the map is re-rendered when placements or the
        # watermark change, and a same-name replacement sits behind browser and CDN
        # caches for days. Bump the date here and upload under the new name.
        "full_map": "/wp-content/uploads/guide-images/collectables/Slasher-Mask-Locations/slasher_masks-2026-09.jpg",
        # The same map with every mask numbered. Pairs with the printable tick-off
        # checklist: the numbers on this map are the numbers on that sheet, both
...
# The channel this RUN is on ('pts' with --pts or DFBNB_CHANNEL=pts, else 'live').
# Every per-channel read in this file goes through it — derived TSVs and the CHAL
# join alike — so a PTS build never mixes in live data.
RUN_CHANNEL = _ts.channel_of()
GRAVE_TSV = _ts.derived_read("phantom_grave_sites.tsv", RUN_CHANNEL)
GRAVE_NOTES_TSV = os.path.join(REPO, "tsv", "phantom_grave_notes.tsv")

#   tsv/collectable_marker_overrides.tsv   EDITORIAL — the fast-travel marker a
#   reader should actually use, for the Mappalachia-resolved sets. See
#   load_marker_overrides() for why the derived name is not always the right one.
MARKER_OVERRIDES_TSV = "collectable_marker_overrides.tsv"

#   tsv/collectable_map_numbers.tsv   GENERATED by src/render_slasher_maps.py — the
#   number each placement carries on the numbered map, so the page's printable
#   checklist agrees with the map a reader prints beside it.
MAP_NUMBERS_TSV = "collectable_map_numbers.tsv"
...
# The hand-authored photo/text slots on one spawn, in render order.
SPAWN_SLOTS = ("image_top", "directions", "image_bottom")
...
def load_existing_spawn_handfills(path):
    """Hand-authored per-spawn values from an existing dist JSON, two ways in:

        by_ref : {ref_formid: {image_top, directions, image_bottom}}
        by_key : {(region, marker, label): {...}}

    A spawn's REF FormID is its only stable identity — a marker can be renamed or
    merged between passes (Mappalachia's name vs an editorial one) and a rebuild
    keyed on the marker alone then silently drops that marker's directions and
    photos. Ref first, (region, marker, label) as the fallback for a spawn with no
    ref. This is the promise the workflow makes about this build: "hand-filled
    directions/images are merge-preserved on rebuild".
    """
    by_ref, by_key = {}, {}
    try:
        data = json.load(open(path, encoding="utf-8"))
    ...
    for reg in data.get("regions", []):
        ...
        for loc in reg.get("locations", []):
            ...
            refs = loc.get("refs") or []
            for i, sp in enumerate(loc.get("spawns") or []):
                saved = {k: sp.get(k, "") for k in SPAWN_SLOTS if sp.get(k)}
                if not saved:
                    continue
                label = sp.get("label") or f"Spawn {i + 1}"
                by_key[(rname, marker, label)] = saved
                if i < len(refs) and refs[i]:
                    by_ref[refs[i]] = saved
    return by_ref, by_key


def load_existing_marker_notes(path):
    """Hand-authored marker-level `note` from an existing dist JSON: (by_ref, by_key).

    A third kind of hand-written content, separate from the per-spawn directions: a
    note about how to RUN the marker — "clear the area first, then fast travel back
    so you start from the spawn-in point". It belongs to the marker, not to any one
    corpse, so it is stored against every REFR at that marker and recovered from
    whichever one survives. (region, marker) is the fallback.
    """
    by_ref, by_key = {}, {}
    try:
        data = json.load(open(path, encoding="utf-8"))
    ...
    for reg in data.get("regions", []):
        ...
        for loc in reg.get("locations", []):
            ...
            if not note:
                continue
            by_key[(rname, loc.get("marker", ""))] = note
...
def load_marker_overrides(slug, channel="live"):
    """tsv/collectable_marker_overrides.tsv -> {ref_formid: {region, marker}} for one set.

    Mappalachia resolves a placement's marker by PROXIMITY, which is the right answer
    for where a thing is and the wrong answer for how a player gets to it. The nearest
    marker is regularly not the one you would fast-travel to: it can be across a ridge,
    be a workshop you cannot travel to, or be further by road than a marker that is
    further as the crow flies. Duchess walks these routes and names the marker that is
    actually fastest to reach the spawn from — that is routing information, not a typo,
    and it must outlive every rebuild.

    Keyed on the REFR FormID, never on the derived marker name, for the same reason the
    hand-fills are: the derived name is the thing being overridden, so keying on it
    would break the override the moment Mappalachia changed its mind. An override also
    MERGES placements onto one marker (two corpses cleared in a single run-in), which a
    name-keyed table cannot express.

    Hand-authored, never written by a build script. Missing file is not an error.
    """
    path = _ts.derived_read(MARKER_OVERRIDES_TSV, channel)
    out = {}
    ...
    with open(path, encoding="utf-8") as fh:
        ...
        for line in fh:
            ...
            if cell("set") != slug:
                continue
            ref = cell("ref_formid").upper()
            if not ref:
                continue
            out[ref] = {"region": cell("region_override"),
                        "marker": cell("marker_override")}
    return out


def load_map_numbers(slug, channel="live"):
    """tsv/collectable_map_numbers.tsv -> {ref_formid: map_number} for one set.

    Written by the map renderer, which owns the numbering: the numbers are assigned
    in ITS traversal order, and this build orders placements differently (region A-Z,
    then marker, then placement), so it cannot recompute them. Reading them keeps the
    printable checklist and the numbered map in agreement — the whole point of the
    checklist being that you tick it off with the map in your other hand.

    Missing file is not an error: the checklist just falls back to page order.
    """
    path = _ts.derived_read(MAP_NUMBERS_TSV, channel)
    out = {}
    if not os.path.exists(path):
        return out
    with open(path, encoding="utf-8") as fh:
        ...
        for line in fh:
            ...
            if cell("set") != slug:
                continue
            ref, n = cell("ref_formid").upper(), cell("map_number")
            if ref and n.isdigit():
                out[ref] = int(n)
    return out
...
def _prune_optional(doc):
    ...
    A set with no numbered map should not carry `"numbered_map": ""` — an always-empty
...
def build_set(slug, rows):
    ...
    handfills = load_existing_handfills(out_path)          # legacy marker-level shape
    hf_by_ref, hf_by_key = load_existing_spawn_handfills(out_path)
    note_by_ref, note_by_key = load_existing_marker_notes(out_path)
    map_numbers = load_map_numbers(slug, RUN_CHANNEL)
    ...
    # Editorial marker/region wins over the proximity-derived one. Applied BEFORE
    # aggregation so an override also merges placements onto a shared marker.
    overrides = load_marker_overrides(slug, RUN_CHANNEL)
    for r in rows:
        region = r.get("region") or ""            # may be "" for a polygon-gap placement
        marker = r.get("marker") or "(unknown location)"
        ov = overrides.get((r.get("ref_formid") or "").strip().upper())
        if ov:
            region = ov["region"] or region
            marker = ov["marker"] or marker
    ...
    for region in REGIONS_AZ:
        ...
        for marker in sorted(by_region.get(region, {}), key=lambda m: marker_sort_key(region, m)):
            agg = by_region[region][marker]
            total += agg["count"]
            # One entry per placement at this marker — the shape the renderer reads
            # ("New model: loc.spawns[] holds one entry per corpse at this marker").
            # Directions and photos are per corpse, so they cannot live on the
            # marker: the marker-level slots below are legacy and are migrated onto
            # Spawn 1 rather than dropped.
            legacy = handfills.get((region, marker), {})
            refs = agg["refs"]
            spawns = []
            for i in range(agg["count"]):
                label = f"Spawn {i + 1}"
                ref = refs[i] if i < len(refs) else ""
                hf = (hf_by_ref.get(ref) if ref else None) \
                     or hf_by_key.get((region, marker, label)) \
                     or (legacy if i == 0 else None) or {}
                sp = {"label": label}
                # The number this placement carries on the numbered map. Emitted only
                # when the map has published one, so a set with no map stays clean.
                if ref and ref.upper() in map_numbers:
                    sp["map_number"] = map_numbers[ref.upper()]
                sp.update({
                ...
                spawns.append(sp)
            # Marker-level routing note, recovered by REFR first so it follows the
            # marker through a rename or a merge. Slotted after `count`, matching
            # the published shape the renderer reads.
            note = next((note_by_ref[r] for r in refs if r in note_by_ref),
                        note_by_key.get((region, marker), ""))
            loc = {"marker": marker, "count": agg["count"]}
            if note:
                loc["note"] = note
            loc.update({
                "spawns": spawns,
    ...
    return _prune_optional(doc), out_path
...
def build_grave_set():
    ...
    # (the graves set builds after the resolved-row sets, so the failure landed at
    # the very end of the job). Read on THIS run's channel: a PTS build joining the
    # live CHAL export is the channel-crossing tsv_source exists to stop.
    challenges = (cchal.challenges_for_set(GRAVE_SLUG, channel=RUN_CHANNEL)
                  or existing_top.get("challenges"))
    if challenges:
    ...
    return _prune_optional(data), out_path
...
DIG_SETS = {
    ...
    "treasure-maps-locations": _ts.derived_read("treasure_map_dig_sites.tsv", RUN_CHANNEL),
    "u-mine-it": _ts.derived_read("u_mine_it_dig_sites.tsv", RUN_CHANNEL),
}
...
def main():
    ...
    # ONE manifest writer for every entry point. A full run used to write
    # manifest_sets in BUILD order while --graves-only / --dig-only rebuilt it from
    # disk in FILENAME order, so the file's set order flipped depending on which
    # build last touched it — pure churn in the diff, and it made "does a clean run
    # reproduce the repo?" impossible to answer. Rebuilding from the on-disk docs
    # also means a run that skipped a set (missing export) cannot drop it.
    if len(manifest_sets) != len(_rebuild_manifest_from_docs()):
        print("[collectable_spawns] note: manifest lists sets this run did not build "
              "— they came from docs already on disk, which is the intent.")
