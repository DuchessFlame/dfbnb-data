...
def clean_desc(raw):
    """Strip the ALL-CAPS marketing/boilerplate runs from an ENTM DESC field.

    Bethesda appends a shouty storefront note to most CAMP DESC strings, always
    fenced by hyphens:

        "Introducing Adelaide… - ALLIES APPEAR ONCE THEIR FURNITURE IS
         PLACED IN C.A.M.P. MODE. -"

    The wording varies per item type (ALLIES / C.A.M.P. ITEMS / PETS / THIS
    ITEM…), so matching each phrase by hand meant a new regex every time a new
    category shipped — which is why the ally descriptions still carried theirs.
    Anything fenced in hyphens that is entirely upper case is a marketing tag by
    construction, so that is what gets removed. The prose is untouched: the
    camp-item-expands spec says clean the DESC, never rewrite it.
    """
    if not raw:
        return ""
    text = raw.strip()

    # Hyphen-fenced ALL-CAPS run, anywhere in the string. "No lower-case
    # letters" is the test, so punctuation, digits and C.A.M.P. dots are fine.
    text = re.sub(r"\s*-\s*[^a-z\-]{8,}?\.?\s*-\s*", " ", text)

    # Same run left dangling at the end with no closing hyphen.
    text = re.sub(r"\s*-\s*[^a-z\-]{8,}\.?\s*$", "", text)

    # Collapse multiple spaces and drop a trailing bare hyphen.
...
# ---------------------------------------------------------------------------
# Shared Build Information + Crafting Requirements
# ---------------------------------------------------------------------------
# The camp-item-expands spec requires EVERY camp-item page to carry the same
# Build Information rows and a Crafting Requirements block. Weather stations
# and repair bots already did; allies, pets, pet furniture, pet apparel,
# fridges and cryos did not, which is why those pages rendered a thinner
# Technical section than collectrons.
#
# Everything here is read from the TSVs — WorkshopCount GLOBs for the limits,
# FURN PRPS for power and the CAMP budget cost, COBJ FVPA for components.
# Nothing is invented: when a value genuinely has no record the row is either
# omitted or spelled out ("As many as your CAMP budget allows"), because a
# guessed number is worse than an honest absence.
#
# NOTE these two helpers are defined AFTER _limit_str but reference lookups
# built further down (furn_by_id / cobj_by_cnam / fvpa_to_array). That is fine
# — they are only ever CALLED from the build_* functions, which run at the
# bottom of the file, long after every module-level map exists.


def camp_build_block(furn_id: str = "", *wc_tokens, extra_lines=None) -> str:
    """Standard Build Information block for a placeable CAMP item.

    furn_id    FURN FormID whose PRPS carry PowerRequired and
               WorkshopBudgetObjectMultiplier. Pass "" for non-placeable
               items (pet apparel) — those rows are then simply left out.
    wc_tokens  WorkshopCount GLOB tokens to try, in order.
    extra_lines  page-specific rows appended verbatim (e.g. Fishing Weather).

    Returns a "Label: value\\n…" string, or "" when there is nothing to say.
    """
    lines = []

    camp, workshop = workshop_count_for(*wc_tokens) if wc_tokens else (None, None)
    if wc_tokens:
    ...
    if furn_id and furn_by_id.get(furn_id):
        ...
        try:
            ...
            _power = _power_raw or "0"
        lines.append(f"Power Required: {_power}")

        _flam_raw = furn_prps_value(furn_id, "WorkshopBudgetObjectMultiplier")
        try:
            _flam = str(int(float(_flam_raw))) if _flam_raw else "1"
        except (ValueError, TypeError):
            _flam = "1"
        lines.append(f"Flamingo Units: {_flam}")
...
def craft_array_for(*form_ids, edid_hint: str = "") -> list:
    """Crafting components for an item, as [{"name":…, "qty":…}, …].

    Tries each FormID against the COBJ CNAM index (borrowing the shared LVLI
    recipe's FVPA when the per-item CondProxy row has none), then falls back
    to a COBJ_EDID suffix match on edid_hint for items whose recipe isn't
    linked by CNAM. Returns [] when no recipe exists — the renderer prints
    "obtained as a complete item" for that, which is the truthful outcome for
    an Atom Shop bundle item.
    """
    for fid in form_ids:
        fid = (fid or "").strip()
        if not fid:
            continue
        for _c in cobj_by_cnam.get(fid, []):
            arr = fvpa_to_array(effective_fvpa(_c))
            if arr:
                return arr

    hint = (edid_hint or "").strip().lower()
    if hint:
        for _c in cobj_rows:
            _ce = (_c.get("COBJ_EDID") or "").strip().lower()
            if not _ce or hint not in _ce:
                continue
            arr = fvpa_to_array(effective_fvpa(_c))
            if arr:
                return arr
...
for _c in cobj_rows:
    ...
    if (re.search(r"COMP_Constructible_CampObject|LiteAlly", _ce, re.I)
        ...
        if _cid and _cid not in _known_ally_cobjs and _cid not in _ALLY_DRIFT_EXCLUDE:
            print(f"  [INFO] new ally auto-built from data: {_cid} {_ce} "
                  f"— add an override in data/camp/allies.json ONLY if a derived "
                  f"field (furn/name/obtain/buff/ally) comes out wrong")
...
def build_allies():
    ...
    for cobj_id, meta in discover_allies().items():
        ...
        # be an outright lie (the game caps you at one ally), and inventing a
        # number is worse. Power + Flamingo Units come off the ally's FURN.
        _ally_build_info = camp_build_block(furn_id)
...
def build_pets():
    ...
    for furn_id, entm_id in SPAWN_FURN_TO_ENTM.items():
        ...
        # Build Information + Crafting Requirements. Limits come from the
        # CAMPPets_WorkshopCount / _CAMP globals (1 pet per camp and per
        # workshop); power and CAMP budget cost from the bed/house FURN.
        _pet_build_info = camp_build_block(furn_id, "CAMPPets_WorkshopCount")
        _pet_craft      = craft_array_for(furn_id, entm_id, edid_hint=furn_edid)
        ...
        items.append({
            ...
            "imageCarousel": carousel,
            "xalgFlags":     xalg,
            "buildInfo":     _pet_build_info,
            "craftingRequirements": _pet_craft,
...
def build_pet_furniture():
    ...
    for entm_id in PET_FURNITURE_ENTM_IDS:
        ...
        items.append({
            ...
            "imageCarousel": carousel,
            "xalgFlags":     xalg,
            # No build rows: these records are ENTM-led (no FURN FormID, so no
            # PowerRequired / WorkshopBudgetObjectMultiplier to read), and the
            # only candidate limit global — PETS_WorkshopCount_CampObjects = 4
            # — carries neither a _CAMP nor a _Workshop suffix, so there is no
            # way to tell which cap it is. Printing it under a guessed label
            # would be worse than leaving the block out. If a suffixed global
            # ever ships, pass its token to camp_build_block here.
            "buildInfo":     "",
...
def build_pet_apparel():
    ...
    for entm_id in PET_APPAREL_ENTM_IDS:
        ...
        items.append({
            ...
            "imageCarousel": carousel,
            "xalgFlags":    xalg,
            # Apparel is worn, not placed — there is no build limit, no power
            # draw and no CAMP budget cost, so buildInfo stays empty and the
            # Technical section shows record IDs only. It IS craftable at the
            # Armor Workbench, so the components are pulled from its COBJ.
            "buildInfo":    "",
            "craftingRequirements": craft_array_for(entm_id, edid_hint=_ap_edid_suffix),
            "cutContent":   is_cut(edid),
        })
