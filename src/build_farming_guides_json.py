...
====================
STATUS: SCAFFOLDED — gap filler for the Farming - Guides category.

NOTES FOR NEXT AI
─────────────────
1. This script should generate dist/farming_guides.json from xEdit TSV exports.
...
4. EXPECTED OUTPUT:
   dist/farming_guides.json — consumed by the JS module df-bnb-farming_guides.js on the website

5. WORKFLOW:
   There is a matching GitHub Actions workflow:
   .github/workflows/build-farming_guides.yml
...
Usage:
  ...
  python build_farming_guides_json.py --tsv-root tsv --outdir dist
"""
...
def main():
    print(f"[build_farming_guides_json.py] Gap filler — real build logic TBD")

    # Ensure output directory exists
    os.makedirs("dist", exist_ok=True)

    # Write empty/placeholder JSON
    out_path = os.path.join("dist", "farming_guides.json")
    with open(out_path, "w") as f:
        json.dump({"_status": "scaffolded", "_category": "Farming - Guides", "items": []}, f, indent=2)

    print(f"[build_farming_guides_json.py] Wrote {out_path}")
