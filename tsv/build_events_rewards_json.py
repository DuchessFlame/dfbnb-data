...
def get_gmrw_rows_for_quest(q):
    ...
    # Cross-quest mapping: some events have GMRW rewards parented to a different quest
    ...
    for gmrw_fid in cross_gmrw_fids:
        ...
        all_rows.extend(gmrw_rows_by_id.get(gmrw_fid, []))
    if all_rows: return all_rows
    # Ref1 fallback: some GMRW rows have no ParentQuestLink but Ref1 points to the quest
    for gmrw_fid, gmrw_list in gmrw_rows_by_id.items():
        for r in gmrw_list:
            for ri in range(1, 6):
                ref_col = r.get(f"Ref{ri}", "")
                if ref_col and qid in ref_col:
                    if gmrw_fid not in seen:
                        seen.add(gmrw_fid)
                        all_rows.extend(gmrw_rows_by_id.get(gmrw_fid, []))
            if all_rows: break
        if all_rows: break
...
EVENT_KEY_ALIASES = {
    ...
    "poweringuppowerstation": ["poweringupthundermt", "poweringup", "mtr07power"],
